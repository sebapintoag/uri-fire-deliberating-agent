...
def evaluate_situation(drone_data = {}, weather_data = {}):
    DANGEROUS_OBJECTS = ['trash', 'dry_grass', 'wood', 'electric_cables', 'gas_installation']
    DANGEROUS_ACTIVITIES = ['smoking', 'bonfire', 'bbq', 'burning_trash', 'suspicious_presence', 'soldering']
    ...
    if len(drone_data) > 0:
        ...
        return { 'type': drone_data_type,
                 'subtype': drone_data['risk_factor'],
                 'risk_rank': risk_rank,
                 'location': drone_data.get('location') }
    
    return { 'type': 'weather', 'subtype': 'trash', 'risk_rank': risk_rank, 'place': weather_data['place'] }
...
def make_decisions(situation_state):
    ...
    if situation_state['type'] == 'weather' and situation_state['risk_factor'] <= 3:
        decisions.append({ 'type': 'monitoring_proposal', 'place': situation_state['place'] })
    
    if situation_state['type'] in ['object', 'activity']:
        if situation_state['subtype'] in ['trash', 'wood', 'dry_grass']:
            decisions.append({ 'type': 'call', 'collaborator': 'trash_collectors', 'location': situation_state.get('location') })
        elif situation_state['subtype'] in ['electric_cables']:
            decisions.append({ 'type': 'call', 'collaborator': 'electric_operators', 'location': situation_state.get('location') })
        elif situation_state['subtype'] in ['gas_installation']:
            decisions.append({ 'type': 'call', 'collaborator': 'gas_operators', 'location': situation_state.get('location') })
        elif situation_state['subtype'] in ['bonfire', 'burning_trash']:
            decisions.append({ 'type': 'call', 'collaborator': 'security', 'location': situation_state.get('location') })
            decisions.append({ 'type': 'call', 'collaborator': 'fire_fighters', 'location': situation_state.get('location') })
        elif situation_state['subtype'] in ['smoking', 'bbq', 'suspicious_presence', 'soldering']:
            decisions.append({ 'type': 'call', 'collaborator': 'security', 'location': situation_state.get('location') })

    # No mather what, if risk_rank is 2 or less government authorities are notified
    if situation_state['risk_rank'] <= 2:
        decisions.append({ 'type': 'call', 'collaborator': 'governor', 'location': situation_state.get('location') })
